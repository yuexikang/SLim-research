...
class MambaEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        # 2. Rearrange features into (B, H*W, C) and (B, W*H, C), two directions
        ...
        # 3. Mamba forward, including another two directions, which is the flip of the original two directions
        x0_hw, x1_hw, x0_wh, x1_wh = self.mamba(x0_hw, x1_hw, x0_wh, x1_wh)

        # 4. Rearrange features back to (B, C, H, W)
        # Same effect as following, but a bit more efficient
    ...
    #     # 2. Rearrange features into (B, H*W, C) and (B, W*H, C), and the reversed direction
    #     x_hw = x.flatten(2).transpose(1, 2)
    #     x_hw_r = x_hw.flip(dims=(-2,))
    #     x_wh = x.permute(0, 2, 3, 1).flatten(1, 2)
    #     x_wh_r = x_wh.flip(dims=(-2,))
    #     x = torch.concat([x_hw, x_hw_r, x_wh, x_wh_r], dim=-2)

    #     # 3. Mamba forward
    #     x = self.mamba(x)

    #     # 4. Rearrange features back to (B, C, H, W)
    #     x_hw, x_hw_r, x_wh, x_wh_r = torch.chunk(x, 4, dim=-2)
    #     x_hw = x_hw.view(x_hw.shape[0], x_shape[0], x_shape[1], -1).permute(0, 3, 1, 2)
    #     x_hw_r = (
    #         x_hw_r.flip(dims=(-2,))
    #         .view(x_hw.shape[0], x_shape[0], x_shape[1], -1)
    #         .permute(0, 3, 1, 2)
    #     )
    #     x_wh = x_wh.view(x_wh.shape[0], x_shape[1], x_shape[0], -1).permute(0, 3, 2, 1)
    #     x_wh_r = (
    #         x_wh_r.flip(dims=(-2,))
    #         .view(x_wh.shape[0], x_shape[1], x_shape[0], -1)
    #         .permute(0, 3, 2, 1)
    #     )

    #     x = x_hw + x_hw_r + x_wh + x_wh_r + _x
