...
class MegaDepthDataset(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.augment_fn = (
            augmentor_builder()
            if self.mode == "train" and augmentor_builder is not None
            else None
        )
    ...
    def __getitem__(self, idx):
        ...
        image0, mask0, scale0 = (
            read_megadepth_gray(
                img_name0, self.img_resize, self.df, self.img_padding, self.augment_fn
            )
            if self.color is False
            else read_megadepth_color(
                img_name0, self.img_resize, self.df, False, self.augment_fn
            )
        )
        # np.random.choice([self.augment_fn, None], p=[0.5, 0.5]))
        image1, mask1, scale1 = (
            read_megadepth_gray(
                img_name1, self.img_resize, self.df, self.img_padding, self.augment_fn
            )
            if self.color is False
            else read_megadepth_color(
                img_name1, self.img_resize, self.df, False, self.augment_fn
            )
        )
        # np.random.choice([self.augment_fn, None], p=[0.5, 0.5]))
