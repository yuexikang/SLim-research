...
from .utils.misc import LayerNorm2d
...
class RecurrentRefinementUnit(nn.Module):
    ...
    def forward(
        ...
        # Input Encoding
        feat = torch.cat(
            [feat0_window, feat1_window], dim=1
        ).contiguous()  # (M, 2*C, H, W)
    ...
    @torch.no_grad
    def initial_forward(self, length, win_size):
        for i in range(10):
            feat0_window = torch.zeros(length, self.input_dim, win_size, win_size).to(
                self.conv_offset[0].weight.device
            )
            feat1_window = torch.zeros(length, self.input_dim, win_size, win_size).to(
                self.conv_offset[0].weight.device
            )
            hidden_state = torch.zeros(length, self.hidden_dim, win_size, win_size).to(
                self.conv_offset[0].weight.device
            )
...
#         self.context_network = (
#             ConvContextBlock(
#                 self.hidden_dim + self.input_dim, self.hidden_dim, kernel_size=7
#             )
#             if context
#             else nn.Identity()
#         )
...
#     def forward(
#         self,
#         feature_coor: torch.Tensor,
#         hidden_state: torch.Tensor,
#     ) -> Tuple[torch.Tensor, torch.Tensor]:
#         """
#         Forward function of Recurrent Refinement Unit
#         Args:
#             feature_coor (torch.Tensor): (M, C, H, W)
#             hidden_state (torch.Tensor): (M, C, H, W)
...
#         # Context injection
#         if self.context:
#             hidden_state = self.context_network(
#                 torch.cat([hidden_state, feature_coor], dim=1)
#             )
#             # hidden_state = self.context_network(h=hidden_state, x=feat)
#         else:
#             flow = self.flow_encoder(feature_coor)  # (M, C, H, W)
#             hidden_state = flow
