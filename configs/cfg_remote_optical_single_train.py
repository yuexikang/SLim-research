...
def get_cfg_defaults():  # 返回“遥感单张影像合成配准”训练配置
    ...
    cfg.MODE = "remote_optical_single_train"  # 当前配置名称，用来标识训练模式
    cfg.PRETRAINED_PATH = "ckpt/megadepth_19epochs.ckpt"  # 从 MegaDepth 预训练权重开始微调
    cfg.BATCH_SIZE = 6  # 每张 GPU 每个 step 输入 n 个样本；单张模式下每个样本会生成 image0/image1
    cfg.IMAGE_SIZE = 512  # 遥感影像统一 resize 到 512x512 后训练
    cfg.AMP = True  # 开启混合精度训练，节省显存并提升速度

    cfg.DEVICE.GPU_IDX = "1,2,"  # 默认使用物理 0 号 GPU；注意带逗号表示指定 GPU id
    cfg.DEVICE.ENABLE_DDP = True  # 默认不开多卡 DDP；多卡时可改 True 或由 train.py 自动处理

    cfg.DATASET.DATA_SOURCE = "RemoteSensing"  # 数据集类型标记为遥感数据
    cfg.DATASET.TRAINVAL_DATA_SOURCE = "RemoteSensing"  # 训练/验证都走遥感数据加载器
    cfg.DATASET.REMOTE_TRAIN_MANIFEST = "data/remote_archive/manifests/train_optical_single_images.jsonl"  # 训练单张光学影像索引
    cfg.DATASET.REMOTE_VAL_MANIFEST = "data/remote_archive/manifests/val_optical_single_images.jsonl"  # 验证单张光学影像索引
    cfg.DATASET.REMOTE_IMAGE_SIZE = cfg.IMAGE_SIZE  # 数据加载器使用的 resize 尺寸，与全局 IMAGE_SIZE 保持一致
    cfg.DATASET.REMOTE_MAX_TRAIN_SAMPLES = 0  # 训练集最多读取多少条；0 表示不截断 manifest
    cfg.DATASET.REMOTE_MAX_VAL_SAMPLES = 0  # 验证集最多读取多少条；0 表示不截断 manifest
    cfg.DATASET.REMOTE_VAL_SPLIT_RATIO = 0.2  # 没有填写验证 manifest 时，从训练 manifest 中按这个比例拆出验证集
    cfg.DATASET.REMOTE_HOMOGRAPHY_DIFFICULTY = 0.30  # 在线合成 homography 的扰动强度，越大变换越难
    ...
    cfg.DATASET.PARALLEL_WORKERS_NUM = 2  # 保留给数据集并行处理的 worker 数字段，当前 DataLoader 主要看 LOADER.NUM_WORKERS
    cfg.TRAINER.ACCUMULATE_GRAD_BATCHES = 1  # 梯度累积步数；>1 时每 accumulate_grad_batches 个 step 才更新一次梯度，等效于增大 batch size

    cfg.SAMPLER.N_SAMPLES_PER_SUBSET = 0  # 每个 epoch 从该数据子集采样多少条；0 表示自动使用当前训练集全部样本
    ...
    cfg.LOADER.NUM_WORKERS = 6  # DataLoader 进程数；正式训练可调到 4/8/16 提升吞吐
    ...
    cfg.MODEL.COARSE_THRES = 0.1  # 粗匹配置信度阈值，越高匹配越少但更严格
    cfg.MODEL.FINE_THRES = 0.1  # 精匹配置信度阈值，越高匹配越少但更严格

    cfg.TRAINER.CANONICAL_BS = 6  # 标称 batch size，用于按实际 batch 自动缩放学习率
    cfg.TRAINER.CANONICAL_LR = 1e-5  # 标称学习率，实际 TRUE_LR 会按 batch size 缩放
    ...
    cfg.TRAINER.MAX_EPOCHS = 20  # 默认训练 27 个 epoch，可被 train.py 的 --max_epochs 临时覆盖
    ...
    cfg.LOGGER.LOGGER_NAME = "remote_optical_single"  # 默认日志任务名，可被 train.py 的 --task_name 覆盖
    ...
    cfg.LOGGER.WANDB_MODE = "offline"  # W&B 模式：online 正常上传，offline 本地缓存，disabled 禁用
    #wandb sync logs/tb_logs/opt_single/lr5e-5_bs6_512_epoch30/wandb/wandb/offline-run-xxxx
    cfg.LOGGER.WANDB_LOG_MODEL = "all"  # 记录 checkpoint 到 W&B；all 表示记录所有 ModelCheckpoint 产物
    cfg.LOGGER.WANDB_TAGS = "remote,optical,single_synth"  # W&B 标签，便于筛选实验
    return cfg  # 返回最终配置
