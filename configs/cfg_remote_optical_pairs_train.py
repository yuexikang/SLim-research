...
def get_cfg_defaults():  # 返回“遥感光学-光学成对影像”训练配置
    ...
    cfg.MODE = "remote_optical_pairs_train"  # 当前配置名称，用来标识训练模式
    cfg.PRETRAINED_PATH = "ckpt/megadepth_19epochs.ckpt"  # 从 MegaDepth 预训练权重开始微调
    cfg.BATCH_SIZE = 6  # 每张 GPU 每个 step 输入 n 对光学-光学遥感影像
    cfg.IMAGE_SIZE = 512  # 遥感影像统一 resize 到 512x512 后训练
    cfg.AMP = True  # 开启混合精度训练，节省显存并提升速度

    cfg.DEVICE.GPU_IDX = "0,2"  # 默认使用物理 0 号 GPU；注意带逗号表示指定 GPU id
    ...
    cfg.DATASET.REMOTE_TRAIN_MANIFEST = "data/remote_archive/manifests/train_jl1flight_gt.jsonl"  # 训练光学-光学成对影像索引；gt 指向原数据集目录下的 3x3 txt
    cfg.DATASET.REMOTE_VAL_MANIFEST = "data/remote_archive/manifests/test_jl1flight_gt.jsonl"  # 验证光学-光学成对影像索引；gt 指向原数据集目录下的 3x3 txt
    ...
    # jl1flight 自带 b_t* 与 gt 矩阵，训练时只使用原始 A/B 和 txt 真值，不再额外在线随机扰动。
    cfg.DATASET.REMOTE_HOMOGRAPHY_DIFFICULTY = 0.0  # 设为 0 时在线扰动 H 为单位矩阵
    cfg.DATASET.REMOTE_LEFT_IDENTITY = True  # 关闭扰动后该项不改变图像；保留 True 便于和其它遥感配置一致
    cfg.DATASET.REMOTE_AUG_VARIANTS = ["mixed"]  # 只保留 1 份样本，避免把已有 gt_pairs 重复 5 倍
    # 如果训练的是“无 gt 或想额外增强”的成对遥感数据，可恢复下面原配置：
    ...
    # cfg.DATASET.REMOTE_AUG_VARIANTS = ["translation", "scale", "yaw", "pitch", "roll"]  # 在线生成的几何变换类型；每条样本会按列表展开
    ...
    cfg.MODEL.COARSE_THRES = 0.2  # 粗匹配置信度阈值，越高匹配越少但更严格
    cfg.MODEL.FINE_THRES = 0.2  # 精匹配置信度阈值，越高匹配越少但更严格

    cfg.TRAINER.CANONICAL_BS = 6  # 标称 batch size，用于按实际 batch 自动缩放学习率
    cfg.TRAINER.CANONICAL_LR = 5e-5  # 标称学习率，实际 TRUE_LR 会按 batch size 缩放
    ...
    cfg.TRAINER.MAX_EPOCHS = 40 # 默认训练 30 个 epoch，可被 train.py 的 --max_epochs 临时覆盖
    ...
    cfg.LOGGER.LOGGER_NAME = "remote_optical_pairs"  # 默认日志任务名，可被 train.py 的 --task_name 覆盖
    ...
    cfg.LOGGER.WANDB_TAGS = "remote,optical,optical_pairs"  # W&B 标签，便于筛选实验
    return cfg  # 返回最终配置
