# 作用：统一测评 Physical Encoder V0 的合成几何能力、多模态论文指标、跨模态保留率以及与官方 SLiM 粗特征的互补性。
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate a standalone Physical Encoder V0 checkpoint.")
    ...
    parser.add_argument("--manifest_split", choices=["train", "val", "test", "all"], default=None)
    parser.add_argument("--ckpt_path", type=Path, required=True)
    parser.add_argument("--model", choices=["physical_full", "tiny_cnn", "physical_no_canon", "physical_single_scale"], default=None)
    ...
    parser.add_argument("--slim_ckpt_path", type=Path, default=Path("ckpt/megadepth_19epochs.ckpt"))
...
def load_physical_checkpoint(path, device, requested_model=None):
    ...
    encoder = build_physical_v0_encoder(model_name)
...
def merge_statistics(accumulator, stats):
    for variant, values in stats.items():
        for key, value in values.items():
            accumulator[f"{variant}/{key}"] += float(value)


def finalize_statistics(accumulator):
    output = {}
    for variant in ("all", "translation", "scale", "yaw", "pitch", "roll"):
        count = accumulator.get(f"{variant}/count", 0.0)
        if count <= 0:
            continue
        output[variant] = {
            "num_correspondences": int(count),
            "R@0": accumulator[f"{variant}/correct0"] / count,
            "R@1": accumulator[f"{variant}/correct1"] / count,
            "positive_similarity": accumulator[f"{variant}/positive"] / count,
            "hard_negative_similarity": accumulator[f"{variant}/hard_negative"] / count,
            "mean_margin": accumulator[f"{variant}/margin"] / count,
            "entropy": accumulator[f"{variant}/entropy"] / count,
            "normalized_entropy": accumulator[f"{variant}/normalized_entropy"] / count,
        }
    return output


def evaluate_synthetic(args, encoder, model_name, hparams, device):
    variants = list(RemoteSensingHomographyDataset.DEFAULT_AUG_VARIANTS)
    ...
    with torch.inference_mode():
        for index in tqdm(range(len(dataset)), desc="Synthetic Physical V0 evaluation"):
            batch = make_batch(dataset[index], device)
            if device.type == "cuda":
                torch.cuda.synchronize(device)
            started = time.perf_counter()
            descriptor0 = encoder(batch["image0"])
            descriptor1 = encoder(batch["image1"])
            if device.type == "cuda":
                torch.cuda.synchronize(device)
            ...
            merge_statistics(accumulator, stats)
    ...
    summary = {
        ...
        "metrics": finalize_statistics(accumulator),
...
def update_crr(accumulator, prefix, physical_prediction, slim_prediction, correspondences, width):
    ...
    for radius, label in ((0, "r0"), (1, "r1")):
        ...
        pp = correct(physical_prediction)
        ff = correct(slim_prediction)
        values = {
            "both_correct": pp & ff,
            "physical_only": pp & ~ff,
            "slim_only": ~pp & ff,
...
def finalize_crr(accumulator, prefixes):
    ...
    for prefix in prefixes:
        ...
        for label in ("r0", "r1"):
            ...
            ff_wrong = values["physical_only"] + values["both_wrong"]
            values["CRR"] = values["physical_only"] / ff_wrong if ff_wrong else 0.0
...
def evaluate_multimodal(args, encoder, model_name, hparams, device):
    ...
    crr = defaultdict(int)
    with torch.inference_mode():
        for index in tqdm(range(len(dataset)), desc="Multimodal Physical V0 evaluation"):
            item = dataset[index]
            ...
            source, target, _ = nearest_neighbors(
                descriptor0, descriptor1, chunk_size=args.chunk_size
            )
            if device.type == "cuda":
                torch.cuda.synchronize(device)
            ...
            points0 = torch.stack(
                [(source % descriptor0.shape[-1]) + 0.5, (source // descriptor0.shape[-1]) + 0.5], dim=1
            ).float() * 8.0
            points1 = torch.stack(
                [(target % descriptor1.shape[-1]) + 0.5, (target // descriptor1.shape[-1]) + 0.5], dim=1
            ).float() * 8.0
            ...
            points0 = points0.cpu().numpy() * np.array([image0.shape[1] / args.image_size, image0.shape[0] / args.image_size])
            points1 = points1.cpu().numpy() * np.array([image1.shape[1] / args.image_size, image1.shape[0] / args.image_size])
            ...
            modality = f"{record.get('modality0', 'unknown')}-{record.get('modality1', 'unknown')}"
            rows.append(
                {
                    "index": index,
                    "id": record.get("id", ""),
            ...
            if slim is not None:
                physical_prediction = row_predictions(descriptor0, descriptor1, args.chunk_size)
                features0, features1 = slim.feature_backbone(batch["image0"], batch["image1"])
                ...
                correspondences = coarse_homography_correspondences(batch, 8)
                update_crr(crr, "overall", physical_prediction, slim_prediction, correspondences, descriptor1.shape[-1])
                update_crr(crr, modality, physical_prediction, slim_prediction, correspondences, descriptor1.shape[-1])

    groups = defaultdict(list)
    for row in rows:
        groups[row["modality_pair"]].append(row)
    overall = aggregate_protocol(rows)
    by_modality = {name: aggregate_protocol(group) for name, group in sorted(groups.items())}
    ...
    summary = {
        ...
        "protocol": {
            "matching": "coarse cosine mutual-nearest-neighbor; no RANSAC",
        ...
        "slim_coarse_complementarity": (
            finalize_crr(crr, ["overall", *sorted(groups)]) if slim is not None else None
        ),
        "checkpoint_hparams": hparams,
    }
    args.output_dir.mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    if not torch.cuda.is_available() or not args.device.startswith("cuda"):
        raise RuntimeError("Physical V0 evaluation requires CUDA.")
    device = torch.device(args.device)
    torch.cuda.set_device(device)
    encoder, model_name, hparams = load_physical_checkpoint(
