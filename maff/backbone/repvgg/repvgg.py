...
class RepVGG_modified(nn.Module):
    def __init__(
        self,
        num_blocks,
        width_multiplier=None,
        ...
        super(RepVGG_modified, self).__init__()
        assert len(width_multiplier) == 4
        self.deploy = deploy
        self.override_groups_map = override_groups_map or dict()
        assert 0 not in self.override_groups_map
...
def create_RepVGG_A1_modified(deploy=False, use_checkpoint=False):
    return RepVGG_modified(
        num_blocks=[2, 4, 14, 1],
        width_multiplier=[1, 1, 1, 2.5],
        override_groups_map=None,
        deploy=deploy,
